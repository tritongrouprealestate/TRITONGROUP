...
interface ScrollChoreographyProps {
  ...
  /** Accessible descriptions, in the same order as `images`. */
  alt?: {
    topLeft: string;
    topRight: string;
    bottomLeft: string;
...
export function ScrollChoreography({
  ...
  // Reduced motion: render the four frames as a static, readable grid. The
  // choreography is decorative; the photographs are the content, so they must
  // still be reachable when motion is switched off.
  if (prefersReducedMotion) {
    const stills: Array<[string, string]> = [
      [images.topLeft, alt?.topLeft ?? ""],
      [images.topRight, alt?.topRight ?? ""],
      [images.bottomLeft, alt?.bottomLeft ?? ""],
      [images.bottomRight, alt?.bottomRight ?? ""],
    ];
    return (
      <div className={cn("grid w-full grid-cols-1 gap-4 sm:grid-cols-2", className)}>
        {stills.map(([src, description], i) => (
          <div key={i} className="aspect-[3/2] overflow-hidden bg-muted">
            <img src={src} alt={description} className="h-full w-full object-cover" />
          </div>
        ))}
      </div>
    );
    ...
    <div ref={containerRef} className={cn("relative h-[300vh] w-full", className)}>
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          >
            <img src={images.topLeft} alt={alt?.topLeft ?? ""} className="h-full w-full object-cover" />
          </motion.div>
          ...
          >
            <img src={images.bottomRight} alt={alt?.bottomRight ?? ""} className="h-full w-full object-cover" />
          </motion.div>
          ...
          >
            <img src={images.bottomLeft} alt={alt?.bottomLeft ?? ""} className="h-full w-full object-cover" />
          </motion.div>
          ...
          >
            <img src={images.topRight} alt={alt?.topRight ?? ""} className="h-full w-full object-cover" />
          </motion.div>
        </div>
      </div>
    </div>
